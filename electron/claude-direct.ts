...
除了斜杠指令外，当用户用自然语言请求以下内容时，也使用 mermaid 输出：
...
### 指令使用规则
1. 必须先使用工具（Read/Grep/Glob/Bash）分析项目或收集信息
2. 使用 \`\`\`mermaid 代码块输出图表，不要输出 Markdown 表格或纯文字列表
3. 可以在 mermaid 代码块前后添加简短的文字说明
4. 图表要完整、清晰，包含合理的节点和关系

### /arch — 架构图
使用 graph TD（自上而下）布局，包含子图（subgraph）表示分层：
\`\`\`mermaid
graph TD
    subgraph 用户界面
        A[React UI] --> B[Markdown Renderer]
    end
    subgraph 通信层
        C[IPC Bridge] --> D[SSE Stream]
    end
    subgraph 核心引擎
        E[Direct API Client] --> F[Tool Executor]
    end
    A --> C
    C --> E
\`\`\`
