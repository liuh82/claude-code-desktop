...
visual 代码块示例：
\`\`\`visual
{"type": "architecture", "title": "系统架构", "icon": "account_tree", "label": "Architecture", "layers": [{"name": "用户界面", "modules": [{"name": "前端", "icon": "web", "color": "primary"}]}, {"name": "后端服务", "modules": [{"name": "API 网关", "icon": "router", "color": "secondary"}, {"name": "后端", "icon": "dns", "color": "secondary"}]}, {"name": "基础设施", "modules": [{"name": "数据库", "icon": "storage", "color": "tertiary"}]}]}
\`\`\`
...
### /arch 指令规则
- 输出 JSON 使用 layers（分层）结构，比扁平 modules 更清晰地展示架构层次
- 示例格式：\`{"type": "architecture", "title": "...", "layers": [{"name": "层名", "modules": [{"name": "模块名", "icon": "...", "color": "primary"}]}]}\`
- 每层的 modules 数组中，每个模块包含 name、icon、color（"primary"|"secondary"|"tertiary"）
- 按架构层次从上到下排列（用户端 → 前端 → API → 后端 → 存储）
- 如果架构没有明显分层，也可以使用扁平 modules 格式：\`{"type": "architecture", "title": "...", "modules": [...]}\`
